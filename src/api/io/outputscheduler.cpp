...
void IO::OutputScheduler::remove_socket(int sockfd) {
        ...
        if (socket != nullptr) {
                ...
                Log::e("could not find socket with sockfd = " +
                       std::to_string(sockfd) + " in order to remove it");
        }
}
        ...
        try {
                ...
                while (!_stopRequested) {
                        ...
                        for (auto index = 0; index < events_number; ++index) {
                                volatile std::lock_guard<std::mutex>
                                    schedule_lock(_scheduling_mutex);
                                Log::i("got " + std::to_string(events_number) +
                                       " events (writing)");
...
IO::OutputScheduler &IO::OutputScheduler::get() {
        if (_instance.get() == nullptr)
                _instance.reset(
                    new OutputScheduler(Storage::settings().max_connections));
        return (*_instance);
}

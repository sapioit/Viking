...
template <class Sock, class Data> class Out : public SocketWatcher<Sock> {
      ...
      public:
        template <class D> static Out *GetInstance() {
                if (Out<Sock, D>::instance_.get() == nullptr) {
                        instance_.reset(new Out());
                }
                return Out<Sock, D>::instance_.get();
        }
        ...
        virtual void Add(Sock sock, const Data &data) {
                try {
                        Lock lock(sync_);
                        schedule_.insert(std::make_pair(sock.GetFD(), data));
        ...
        virtual void Remove(const Sock &sock) {
                try {
                        ...
                        SocketWatcher<Sock>::Remove(sock);
                } catch (const SysEpoll::Error &) {
                        throw;
                }
        }
        ...
        virtual void Run() noexcept {
                try {
                        ...
                        while (true) {
                                ...
                                if (schedule_.size() != 0) {
                                        auto events =
                                            SysEpoll::Wait(schedule_.size());
                                        debug("Out scheduler has " +
                                              std::to_string(events.size()) +
                                              " events");
                                        auto available_sockets =
                                            this->MatchEventsWithSockets(
                                                events);
                                        for (const auto &pair :
                                             available_sockets) {
                        ...
                        debug("Connection closed by peer on fd = " +
                              std::to_string(e.fd));
                        Remove(*e.ptr);
                }
        }

        virtual void ProcessEvent(const Sock &sock,
                                  const SysEpoll::Event &event,
                                  SchedItem<Data> &sched_item) noexcept {

                static constexpr auto flag_termination =
                    static_cast<const std::uint32_t>(
                        SysEpoll::Description::Termination);
                static constexpr auto flag_error =
                    static_cast<std::uint32_t>(SysEpoll::Description::Error);
                static constexpr auto flag_write =
                    static_cast<std::uint32_t>(SysEpoll::Description::Write);

                if (event.description & flag_termination ||
                    event.description & flag_error) {
                        Remove(sock);
                        return;
                }
                try {
                        if (event.description & flag_write) {
                                ...
                                if (written <= data.size()) {
                                        ...
                                        // data.resize(old_data_size - written);

                                        Data(data.begin() + written, data.end())
                                            .swap(data);
                                        /*

                                        std::vector<char>(
                                            _schedule[scheduled_item_pos].data.begin()
                                        + written,
                                            _schedule[scheduled_item_pos].data.end())
                                            .swap(_schedule[scheduled_item_pos].data);*/

                                        assert(old_data_size - written ==
                                               data.size());
                                }
                        }
                } catch (...) {

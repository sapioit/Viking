...
Request Parser::Init() {
        try {
                ...
                try {
                        request.method = GetMethod(std::string(
                            line.begin(), line.begin() + first_space));
                } catch (std::runtime_error &ex) {
                        throw;
                }
                try {
                        auto _URI_begin =
                            line.find_first_not_of(space, first_space);
                        auto _URI_end =
                            line.find_first_of(space, _URI_begin + 1);
                        auto _version_begin =
                            line.find_first_not_of(space, _URI_end);
                        auto _version_end =
                            line.find_first_of("\r\n", _version_begin + 1);
                        request.URI = std::string(line.begin() + _URI_begin,
                                                  line.begin() + _URI_end);
                        std::string http_version_str(
                            line.begin() + _version_begin,
                        ...
                        request.setUri_components(Parser::Split(
                            Parser::StripRoute(request.URI), '/'));
                        return request;
...
Header Parser::GetHeader() {
        ...
        while (true) {
                ...
                auto key = std::string(line.begin() + key_begin,
                                       line.begin() + key_end);
                auto value = std::string(line.begin() + value_begin,
                                         line.begin() + value_end);

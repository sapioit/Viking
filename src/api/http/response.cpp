...
DFA<states, transitions> make_machine() {
        ...
        machine.add(
            std::make_pair(states::StatusLine, transitions::EndStatusLine),
            states::GeneralHeader);
        machine.add(std::make_pair(states::GeneralHeader, transitions::Error),
                    states::CRLFBody);
        machine.add(std::make_pair(states::GeneralHeader,
                                   transitions::EndGeneralHeader),
                    states::ResponseHeader);
        machine.add(std::make_pair(states::ResponseHeader,
                                   transitions::EndResponseHeader),
                    states::CRLFHeader);
...
std::string Response::str() const {
        ...
        while (machine.currentState() != end) {
                ...
                case states::ResponseHeader: {
                        ...
                        stream << Http::Header::Fields::Content_Type << ": "
                               << type_str << crlf;
                        ...
                        stream << crlf;
                        stream
                            << Http::Header::Fields::Cache_Control << ": "
                            << (should_cache()
                                    ? "max-age=" + std::to_string(get_expiry())
                                    : "no-cache")
                            << crlf;
                        if (has_resource()) {
                                if (getContent_type() ==
                                        Components::ContentType::TextHtml ||
                                    getContent_type() ==
                                        Components::ContentType::TextPlain) {
                                        // stream <<
                                        // Http::Header::Fields::Transfer_Encoding
                                        // << ": " <<
                                        // "8bit";
                                } else {
                                        stream << Http::Header::Fields::
                                                      Transfer_Encoding
                                               << ": "
                                               << "binary" << crlf;
                                }
                ...
                case states::Body: {
                        if (has_resource()) {
                                std::copy(_resource.content().begin(),
                                          _resource.content().end(),
...
bool Response::should_close() const {
        auto connection =
            _request.header.fields.find(Header::Fields::Connection);
